...
function FlyoverMode() {
  ...
  useEffect(() => {
    ...
    console.log("[FlyoverMode:init] Input listeners initialized");
    return cleanup;
  }, []);
  ...
  const handleCityReady = useCallback(() => {
    ...
    console.log("[FlyoverMode:ready] City loaded");
  }, [setCityReady]);
    ...
    >
      ...
      {!showIntro && (
        <>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.6 }} className="absolute top-28 left-4 z-40">
            <CityMiniMap playerPos={[playerPos[0], playerPos[1], playerPos[2]]} buildings={visibleBuildings} userBuildingId={userBuilding?.id} />
          </motion.div>
...
export default function CityExplore() {
  const viewMode = useGameStore(s => s.ui.cityViewMode);

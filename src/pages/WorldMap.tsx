    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="pt-14 flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="w-full lg:w-96 border-r border-gray-800 bg-gray-900/50 flex flex-col">
          ...
          {/* Breadcrumbs */}
          <div className="px-4 py-2 border-b border-gray-800/50 flex items-center gap-1 flex-wrap">
            {breadcrumbs.map((b, i) => (
              <span key={i} className="flex items-center gap-1">
                {i > 0 && <ChevronRight className="w-3 h-3 text-gray-600" />}
                <button onClick={b.onClick} className="text-[10px] text-gray-400 hover:text-white transition-colors" style={{ fontFamily: "monospace" }}>
                  {b.label}
                </button>
              </span>
            ))}
            ...
            {search ? (
              ...
              WORLD_DATA.map(cont => (
                <button
                  ...
                  className="w-full rounded-xl p-4 bg-gray-900/50 border border-gray-800 hover:bg-gray-800/50 hover:border-gray-700 transition-all text-left"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-2xl">{cont.emoji}</span>
                      <div>
                        <h3 className="font-display font-semibold text-white text-sm">{cont.name}</h3>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-display font-semibold text-white text-sm">{country.name}</h3>
                        <p className="text-[10px] text-gray-500">{country.cities.length} cidades</p>
                      </div>
                    </div>
                    <ChevronRight className="w-4 h-4 text-gray-600" />
                  </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-8">
            <div className="relative w-full max-w-4xl aspect-[2/1]">
              ...
              {allCities.map((city) => (
                ...
                >
                  <div className="relative">
                    <div className="w-3 h-3 rounded-full border-2 border-gray-950" style={{ backgroundColor: city.color, boxShadow: `0 0 12px ${city.color}80` }} />
          ...
          {selectedCity && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="absolute bottom-6 right-6 w-96 bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-2xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs">{selectedCity.country}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-gray-800/50 rounded-xl p-3">
                  ...
                  <div className="text-[10px] text-gray-500">jogadores</div>
                </div>
                <div className="bg-gray-800/50 rounded-xl p-3">
                  <div className="text-lg font-display font-bold text-white">{selectedCity.buildings}/{selectedCity.maxBuildings}</div>
                  <div className="text-[10px] text-gray-500">prédios</div>
                </div>
              </div>

              {/* Districts */}
              <div className="mb-4">
                <span className="text-[10px] text-gray-400 tracking-widest font-bold block mb-2" style={{ fontFamily: "monospace" }}>DISTRITOS</span>
                <div className="flex flex-wrap gap-1.5">
                  {selectedCity.districts.map(d => (
                    <span key={d.id} className="text-[9px] px-2 py-1 rounded-md border border-gray-700 text-gray-400" style={{ fontFamily: "monospace" }}>
                      {d.emoji} {d.name}
                    </span>
                  ))}
                </div>
              ...
              {/* Servers */}
              <div className="mb-4">
                <span className="text-[10px] text-gray-400 tracking-widest font-bold block mb-2" style={{ fontFamily: "monospace" }}>SERVIDORES</span>
                  ...
                  {selectedCity.servers.map(s => (
                    <div key={s.id} className="flex items-center justify-between bg-gray-800/30 rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-300" style={{ fontFamily: "monospace" }}>{s.name}</span>
                      </div>
                      <div className="flex items-center gap-3 text-[10px]" style={{ fontFamily: "monospace" }}>
                        <span className="text-gray-500"><Users2 className="w-3 h-3 inline mr-0.5" />{s.population}/{s.maxPopulation}</span>
                        <span className="text-emerald-400">{s.ping}ms</span>
                      </div>
                    </div>
                  ))}
                </div>
...
function CityCard({ city, selected, onClick }: { city: City; selected: boolean; onClick: () => void }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className={`rounded-xl p-4 cursor-pointer transition-all border ${selected ? "bg-gray-800 border-primary/50" : "bg-gray-900/50 border-gray-800 hover:bg-gray-800/50 hover:border-gray-700"}`}
      onClick={onClick}
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-gray-500 text-[10px]">{city.country} · {city.servers.length} server{city.servers.length > 1 ? "s" : ""}</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
          <span className="text-[10px] text-emerald-400">{bestServer?.ping || 0}ms</span>
        </div>
      </div>

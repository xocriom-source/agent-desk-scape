...
const STATUS_CONFIG = {
  ...
  delegated: { icon: ArrowRight, label: "Delegado", color: "text-[#FFB347]", bg: "bg-[#FFB347]/10" },
};

const PRIORITY_COLORS = {
  low: "bg-muted/50 text-muted-foreground",
  medium: "bg-primary/15 text-primary",
  high: "bg-[#FFB347]/15 text-[#FFB347]",
  critical: "bg-destructive/15 text-destructive",
};

function generateInitialTasks(agents: Agent[]): AgentTask[] {
  const templates = [
    { title: "Analisar dados de sentimento", desc: "Processar dados não estruturados e extrair insights", priority: "high" as const },
    { title: "Escrever relatório semanal", desc: "Compilar atividades e progressos da semana", priority: "medium" as const },
    ...
    { title: "Compor trilha para o escritório", desc: "Música ambiente lo-fi para produtividade", priority: "low" as const },
    { title: "Revisar código do SDK v2", desc: "Code review completo com sugestões", priority: "medium" as const },
    { title: "Testar integração de APIs", desc: "Validar todos os endpoints da v3", priority: "high" as const },
    { title: "Documentar processos internos", desc: "Criar wiki com procedimentos padrão", priority: "medium" as const },
    { title: "Automatizar testes E2E", desc: "Cobertura de 90% nos fluxos críticos", priority: "critical" as const },
    { title: "Pesquisar novas técnicas de NLP", desc: "Avaliar modelos para próximo ciclo", priority: "medium" as const },
  ];

  return templates.map((t, i) => {
    const agent = agents[i % agents.length];
    const delegatorAgent = agents[(i + 3) % agents.length];
    ...
    return {
      ...
      assignedBy: i % 3 === 0 ? "Chefe" : delegatorAgent.name,
      status,
      priority: t.priority,
      createdAt: new Date(Date.now() - Math.random() * 86400000 * 5),
      completedAt: status === "completed" ? new Date(Date.now() - Math.random() * 86400000) : undefined,
      delegatedTo: status === "delegated" ? agents[(i + 2) % agents.length].name : undefined,
    };
  });
}
...
export function TaskBoard({ agents, isOpen, onClose }: TaskBoardProps) {
  const [tasks, setTasks] = useState<AgentTask[]>(() => generateInitialTasks(agents));
  ...
  const updateStatus = (taskId: string, status: AgentTask["status"]) => {
    setTasks((prev) =>
      prev.map((t) =>
        t.id === taskId
                        ...
                        {(task.status === "pending" || task.status === "in_progress") && (
                          <>
                            ...
                            <button
                              ...
                              className="p-1 rounded bg-[#FFB347]/10 hover:bg-[#FFB347]/20 transition-colors"
                              title="Delegar"
                            >
